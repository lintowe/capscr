...
impl HdrCapture {
    ...
    pub fn capture_hdr(&self) -> Result<RgbaImage> {
        #[cfg(target_os = "windows")]
        {
            ...
            let (raw_data, width, height, format) = windows_hdr::capture_hdr_screen()?;
                ...
                while let Ok(output) = adapter.EnumOutputs(output_idx) {
                    ...
                    if let Ok(output6) = output.cast::<IDXGIOutput6>() {
                        if let Ok(desc1) = output6.GetDesc1() {
                            ...
                            let is_hdr = color_space.0 == 12 || color_space.0 == 14;

                            if is_hdr {
                                return Ok(HdrDisplayInfo {
                                    is_hdr_enabled: true,
                                    format: if color_space.0 == 12 {
                                        HdrFormat::Hdr10
                                    } else {
                                        HdrFormat::ScRgb
                                    },
