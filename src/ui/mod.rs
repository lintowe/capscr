...
use crate::capture::{CaptureMode, HdrCapture, ToneMapOperator, WindowCapture, WindowInfo};
    ...
    pub fn update(&mut self, message: Message) -> Task<Message> {
        match message {
            ...
            Message::ShowWindowPicker => {
                ...
                return Task::perform(
                    async {
                        WindowCapture::list_application_windows().unwrap_or_default()
                    },
            ...
            Message::SetGifFps(val) => {
                self.config.capture.gif_fps = val.max(1).min(60);
            }
            ...
            Message::UploadComplete(result) => {
                ...
                match result {
                    Ok(url) => {
                        ...
                        if self.config.ui.show_notifications {
                            let _ = show_notification("Upload Complete", &url);
                        }
                    }
                    Err(e) => {
                        if self.config.ui.show_notifications {
    ...
    fn perform_capture(&mut self, mode: CaptureMode) -> Task<Message> {
        match mode {
            ...
            CaptureMode::Region => {
                Task::perform(
                    async move {
                        use crate::capture::{Capture, Rectangle, RegionCapture, ScreenCapture};
                        ...
                        let capture = RegionCapture::new(region);
                        capture.capture()
                    },
                    |result| match result {
                        Ok(image) => Message::ImageCaptured(CapturedImage {
    ...
    fn capture_window(&mut self, window_id: u32) -> Task<Message> {
        Task::perform(
            async move {
                ...
                let capture = WindowCapture::new(window_id);
                capture.capture()
            },
            |result| match result {
                Ok(image) => Message::ImageCaptured(CapturedImage {
    ...
    fn toggle_gif_recording(&mut self) -> Task<Message> {
        match self.recording_state {
            RecordingState::Idle => {
                ...
                let mut recorder = GifRecorder::new(settings);
